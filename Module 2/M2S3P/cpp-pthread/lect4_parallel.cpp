...
int main() {
    ...
    for(int parti = 2; parti <= 6; parti+=2)
    {
    ...
    pthread_t threads[parti];
    ThreadData threadData[parti];


    int chunk_size = size / parti;
    for (int i = 0; i < parti; i++) {
        threadData[i].v1 = v1;
        threadData[i].v2 = v2;
        threadData[i].v3 = v3;
        threadData[i].start = i * chunk_size;
        threadData[i].end = (i == parti - 1) ? size : (i + 1) * chunk_size;
